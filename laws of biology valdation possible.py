...
def reward_function(n, alpha):
    return alpha * (goal - n)

def punishment_function(n, beta):
    return beta * n

def adjust_activation_function(current_activation, n, alpha, beta, goal):
    reward = reward_function(n, alpha)
    punishment = punishment_function(n, beta)
    return current_activation + reward - punishment
...
for iteration in range(iterations):
    ...
    selected_food_option = random.randint(1, NUM_FOOD_OPTIONS)
    energy_cost = selected_food_option * ENERGY_COST
    energy -= energy_cost
    food_energy = selected_food_option * (INITIAL_ENERGY / NUM_FOOD_OPTIONS)
    energy += food_energy
    ...
    print(f"  Selected Food Option: {selected_food_option}")
